...
- A factory-fresh device already has DHCP set as its NVRAM default
  (`lan_ip_mode=1`), so we don't pass `--lan-ip-mode`. (For a static
  IP instead, see [Configure a Squeezebox for static
  IP](/how-to/configure-for-static-ip).)
- We don't pass `--interface` either: when the device reports its
  factory-default interface sentinel and you omit `--interface`,
  `go-udap set` substitutes wired (and prints a stderr notice).
  Requires go-udap v2.1.0+; on earlier versions, add `--interface 1`
  explicitly.
