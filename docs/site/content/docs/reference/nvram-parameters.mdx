...
| `interface` | Integer (0-1) | 0 = Wireless, 1 = Wired. Factory-fresh devices report an "unset" sentinel that firmware refuses to act on; `go-udap set` substitutes wired by default (or wireless if `wireless_SSID` is supplied) and reports the choice on stderr. Pass `--interface` explicitly to suppress the notice. |
