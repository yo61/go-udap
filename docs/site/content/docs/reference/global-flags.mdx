...
| `--interface NAME` | — | Bind discovery (and all subsequent operations) to one named network interface. See [`interfaces`](./commands/interfaces.mdx) for valid names. Mutually exclusive with `--all-interfaces`. **Not supported on Windows** — the flag is hidden from `--help` there |
